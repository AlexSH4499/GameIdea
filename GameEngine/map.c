...
void generateGridCell(map_strct* r, gridCell_strct* gc, u16 x, u16 y)
{
	if(!r || !gc)return;
	...
	while(lc)
	{
		if((abs(x-(lc->data.pos.x*2+lc->data.size.x))<=(CELLSIZE+abs(lc->data.size.x))) && (abs(y-(lc->data.position.z*2+lc->data.size.z))<=(CELLSIZE+abs(lc->data.size.z))))
		{
			gc->numRectangles++;
		}
