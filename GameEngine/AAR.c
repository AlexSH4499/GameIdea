...
void toggleAAR(unsigned short id)
{
...
void generateGrid(grid* g)
{
    ...
    static unsigned short temp[NUMAARS];
...
for(i=0;i<g->width;i++)
	{
		for(j=0;j<g->height;j++)
		{
			...
			if(n->length){n->data=allocateData(sizeof(unsigned short)*n->length);memcpy(n->data,temp,n->length*sizeof(u16));}
			else n->data=NULL;
		}
	}
